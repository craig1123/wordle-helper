...
})({"jrchc":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "ff36a2c8cf111976";
...
},{}],"4TyOV":[function(require,module,exports) {
...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    for(let i1 = 1; i1 <= 5; i1++){
        const letterEl = document.getElementById(`letter${i1}`);
    ...
    function onKeyUp(e) {
        ...
        const nextInput = document.getElementById(`letter${currentIndex + 1}`);
        if (nextInput) nextInput.focus();
        else form.elements[6].focus(); // button
    }
    function onRestart() {
        guessedWords = [];
        wordCount.textContent = "Possible Words";
        Array.from(guesses.children).forEach((child)=>{
            if (child.className === "row guess-row") child.remove();
        });
    ...
    function onSubmit(e) {
        ...
        for(let i = 0; i < 5; i++){
            ...
            word1.push({
                letter: value,
                valid: Number(dataset.valid)
            });
        }
        if (word1.length !== 5) return;
        const row = document.createElement("div");
        row.className = "row guess-row";
        word1.forEach((el)=>{
            const tile = document.createElement("div");
            let validClass = "";
            if (el.valid === 2) validClass = "correct";
            else if (el.valid === 1) validClass = "present";
            tile.className = `tile small-tile ${validClass}`;
            tile.dataset.valid = el.valid;
            tile.textContent = el.letter;
            row.appendChild(tile);
        });
        guessedWords.push(word1);
        guesses.appendChild(row);
        resetForm();
        const possibleWordles = wordle(word1, guessedWords);
...
function wordle(input, guessedWords) {
    let possibleWords = [];
    const incorrectGuessedLetters = guessedWords.flat().map((word)=>word.valid === 0 ? word : null
    ).filter(Boolean);
    const validLetters = guessedWords.map((words1)=>words1.map((word, index)=>{
            if (word.valid === 0) return null;
            else if (word.valid === 2) return {
                ...word,
                index
            };
            return word;
    ...
    words.filter((word)=>{
        ...
        incorrectGuessedLetters.forEach(({ letter  })=>{
            if (hasGuessedLetter && word.includes(letter)) hasGuessedLetter = false;
        });
        ...
        for(let i = 0; i < input.length; i++){
            ...
            if (el.valid === 1 && letterIsInWord) {
                ...
                pushIt = false;
                break;
            } else if (el.valid === 2 && letterIsInWord && word[i] === el.letter) pushIt = true; // all words have to have the letter in the exact place
            else if (el.valid === 2 && letterIsInWord && word[i] !== el.letter) {
                pushIt = false;
                break;
            } else if (el.valid === 0 && !letterIsInWord) pushIt = true;
        }
        // word needs to have all yellow and green letters in word
        let hasAll = true;
        validLetters.forEach(({ valid , letter , index  })=>{
            if (!word.includes(letter)) hasAll = false;
            else if (valid === 2 && word[index] !== letter) hasAll = false;
        });
        if (hasAll && pushIt) possibleWords.push(word);
    });
...
function mostCommonLetters(possibles, used) {
    ...
    possibles.forEach((word)=>{
        for(let i = 0; i < word.length; i++){
            ...
            if (mapOfLetters[letter]) mapOfLetters[letter]++;
            else mapOfLetters[letter] = 1;
        }
    });
    return Object.keys(mapOfLetters).sort((a, b)=>mapOfLetters[b] - mapOfLetters[a]
    ).map((letter)=>({
            [letter]: mapOfLetters[letter]
        })
    );
}
...
function isBestWordAPossible(possibles, bests) {
    const morePossible = [];
    possibles.forEach((word)=>{
        if (bests.includes(word)) morePossible.push(word);
    });
    return morePossible;
}

},{"./words.js":"gEDpQ","./allWords.js":"6xtf0"}],"gEDpQ":[function(require,module,exports) {
const words = [
    'cigar',
    'rebut',
    'sissy',
...
},{}],"6xtf0":[function(require,module,exports) {
const allWords = [
    'cigar',
    'rebut',
    'sissy',
...
},{}]},["jrchc","4TyOV"], "4TyOV", "parcelRequireea44")

//# sourceMappingURL=index.cf111976.js.map
